...
#### 엄밀한 Red 단계 절차 (컴포넌트 / 모듈 신규 생성 시)
Red 단계는 "테스트가 실행조차 안 되는 상태"가 아니라 "테스트가 실행되어 assertion이 실패하는 상태"여야 한다.
모듈이 존재하지 않으면 Jest가 `Test suite failed to run` 에러를 내며 어떤 케이스가 왜 실패하는지 알 수 없다.

따라서 테스트 파일 작성 직후, Green 단계로 넘어가기 전에 **최소 stub**을 만든다:

```tsx
// 컴포넌트 예시
export function TicketCard(_props: unknown) {
  return null;
}
```
...
```ts
// 함수/서비스 예시
export function myFunction(..._args: unknown[]): never {
  throw new Error('not implemented');
}
```

stub 조건:
- export 이름과 시그니처가 테스트의 import와 일치할 것
- 내부 로직은 없거나 `throw new Error('not implemented')`
- stub 작성 후 테스트를 실행해 모든 케이스가 **assertion 에러**로 실패하는지 확인
- 이 확인이 끝난 뒤에만 Green 단계(구현)로 진입
