...
| 002-4 | Done 24시간 필터 | completedAt이 24시간 이내인 DONE 티켓 | Done 칼럼에 포함 |
| 002-5 | Done 24시간 초과 | completedAt이 24시간 이전인 DONE 티켓 | Done 칼럼에서 제외 |
| 002-6 | isOverdue 파생 필드 | 각 티켓에 isOverdue 포함 | boolean 값 확인 |
| 002-7 | 모든 날짜 필드 포함 | 정상 조회 | plannedStartDate, dueDate, startedAt, completedAt 포함 |
| 002-8 | 응답 구조 확인 | 정상 조회 | 최상위 키가 BACKLOG, TODO, IN_PROGRESS, DONE 4개만 존재 (total 등 래퍼 없음) |
...
| 003-4 | isOverdue 미포함 | 정상 조회 | 응답에 isOverdue 필드 없음 (보드 조회 전용)
...
| 005-3 | position 요청값 반영 | `{ status: "DONE", position: 0 }` | 응답의 position이 요청값과 동일 (서버 자동 계산 아님) |
...
| 007-8 | 응답은 이동된 단건 티켓 | 정상 이동 | 응답 바디가 이동된 티켓 객체 1개 (배열 아님) |
