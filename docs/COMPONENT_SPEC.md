...
| complete | `PATCH /api/tickets/:id/complete` | Done으로 이동, completedAt 자동 설정 (position 필요) |
...
사용자 드롭
  → onDragEnd:
    ...
    2-a. [대상 = Done]
        ...
        → useTickets.complete(ticketId, position)
        → PATCH /api/tickets/:id/complete  { status: "DONE", position }
        → completedAt 자동 설정
...
| US-004: 마감 초과 인지 | TicketCard | useTickets (조회), isOverdue |
